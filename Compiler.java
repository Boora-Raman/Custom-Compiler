...
//         }
//         errors.add("Error at line " + current.line + ", column " + current.column + 
//             ": Expected 'call', 'return', 'if', or assignment");
//         return null;
//     }
...
//         Token current = currentToken();
//         if (current != null && "KEYWORD".equals(current.type) && "call".equals(current.value)) {
//             return parseFunctionCall();
//         }
...
//         Token nameToken = tokens.get(pos - func.children.size() - 1); // Approximate token position
//         String funcName = func.value;
//         symbolTable.add(funcName, "Function", nameToken.line, nameToken.column);

//         ASTNode params = func.children.get(0);
//         List<String> paramTypes = new ArrayList<>();
//         for (ASTNode param : params.children) {
//             symbolTable.add(param.value, "Double", nameToken.line, nameToken.column + funcName.length() + 2);
...
//             }
//         }
//         symbolTable.add(funcName, returnType, nameToken.line, nameToken.column);
//     }

//     private void analyzeAssignment(ASTNode assignment) {
//         Token idToken = tokens.get(pos - assignment.children.size() - 2); // Approximate token position
//         String varName = assignment.value;
//         ASTNode expr = assignment.children.get(0);
//         String type = inferReturnType(expr);
//         symbolTable.add(varName, type, idToken.line, idToken.column);
//     }

//     private void analyzeFunctionCall(ASTNode call) {
//         Token idToken = tokens.get(pos - call.children.size() - 2); // Approximate token position
...
//             output.append("        ").append(type).append(" ").append(varName).append(";\n");
//         }

//         // Generate assignments and print calls
//         for (ASTNode element : ast.children) {
//             if ("Assignment".equals(element.type)) {
//                 String varName = element.value;
...
//             } else if ("FunctionCall".equals(element.type) && "print".equals(element.value)) {
...
// You're correct—despite the updates, the variables zz and ww are still not being properly declared in the main method of the generated CompilerOutput.java. The issue arises because the current code only declares variables from assignments with literals (x = 10.5;, y = 20;) but fails to declare variables assigned via function calls (zz = call add(x, y);, ww = call subtract(x, y);). Additionally, the semantic analysis and code generation phases need to correctly propagate the line/column information and ensure all variables are declared.

// Let’s fix this by ensuring:

// All variables assigned in the program (whether from literals or function calls) are declared in the main method.
// The correct line and column numbers are tracked in the symbol table (currently showing as 0, 0).
// The generated code avoids the "undefined variable" errors for zz and ww.

// └──╼ $java Compiler
// Added to symbol table: x (Type: Double, Line: 8, Column: 12)
// Added to symbol table: y (Type: Double, Line: 8, Column: 12)
// Added to symbol table: add (Type: Function, Line: 8, Column: 12)
// Added to symbol table: x (Type: Double, Line: 8, Column: 17)
// Added to symbol table: y (Type: Double, Line: 8, Column: 17)
// Added to symbol table: add (Type: Double, Line: 8, Column: 12)
// Added to symbol table: subtract (Type: Function, Line: 8, Column: 11)
// Added to symbol table: x (Type: Double, Line: 8, Column: 21)
// Added to symbol table: y (Type: Double, Line: 8, Column: 21)
// Added to symbol table: subtract (Type: Double, Line: 8, Column: 11)
// Compilation complete. Output written to CompilerOutput.java and tokens.txt
...
// /* Compilation Errors:
// Error at line 6, column 1: Expected ; but found ww
// Error at line 5, column 4: Expected ';' after assignment
// Error at line 6, column 3: Expected assignment, function definition, or 'call'
// */
...
public class Compiler {
    ...
    private void analyzeFunctionCall(ASTNode call) {
        String funcName = call.value;
        Token idToken = tokens.stream().filter(t -> t.value.equals(funcName) && "IDENTIFIER".equals(t.type)).findFirst().orElse(null);
        int line = idToken != null ? idToken.line : 0;
        int column = idToken != null ? idToken.column : 0;
        if (!symbolTable.contains(funcName) && !"print".equals(funcName)) {
    ...
    private String inferReturnType(ASTNode expr) {
        ...
        if ("Variable".equals(expr.type)) return symbolTable.getType(expr.value);
        if ("FunctionCall".equals(expr.type)) return symbolTable.getType(expr.value);
        if ("BinaryOp".equals(expr.type)) return "Double";
        if ("Comparison".equals(expr.type)) return "Boolean";
        return "Double";
    }
